...
exports.SmetaDB = class {
  ...
  static async deleteSmeta(params) {
    const tables = await db.query(
      `SELECT conrelid::regclass AS name FROM  pg_constraint WHERE confrelid = 'smeta'::regclass`
    );
    for (let table of tables) {
      const test = await db.query(
        `SELECT * FROM ${table.name} WHERE smeta_id = $1 AND isdeleted = false`,
        params
      );
      if (test.length > 0) {
    ...
    const result = await db.query(
      `UPDATE smeta SET isdeleted = true WHERE id = $1 RETURNING id`,
      params
    );
