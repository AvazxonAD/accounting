...
exports.PrixodBookService = class {
  static async getUniqueSchets(data) {
    const result = await PrixodBookDB.getUniqueSchets([]);

    return result;
  }

  static async getMainSchets(data) {
    const result = await PrixodBookDB.getMainSchets([
      data.region_id,
      data.budjet_id,
    ]);
  ...
  static async delete(data) {
    await db.transaction(async (client) => {
      await PrixodBookDB.delete([data.id], client);

      await PrixodBookDB.deleteChildByParentId([data.id], client);
    });
  }

  static async gerPrixodBookType(data) {
    const result = await PrixodBookDB.gerPrixodBookType([]);
  ...
  static async create(data) {
    const result = await db.transaction(async (client) => {
      const doc = await PrixodBookDB.create(
        [
          1,
          data.accept_time,
          new Date(),
  ...
  static async update(data) {
    const result = await db.transaction(async (client) => {
      ...
      // doc update
      const doc = await PrixodBookDB.update(
      ...
      for (let child of data.old_data.childs) {
        for (let sub_child of child.sub_childs) {
          ...
          if (!check) {
            await PrixodBookDB.deleteChild([sub_child.id], client);
          }
        }
      }
      ...
      for (let child of data.childs) {
        for (let sub_child of child.sub_childs) {
          if (sub_child.id) {
            await PrixodBookDB.updateChild(
              [
                sub_child.schet,
                sub_child.prixod,
                sub_child.prixod,
  ...
  static async createChild(data) {
    for (let child of data.childs) {
      for (let sub_child of child.sub_childs) {
        await PrixodBookDB.createChild(
          [
            sub_child.schet,
            sub_child.prixod,
            sub_child.prixod,
  ...
  static async getById(data) {
    const result = await PrixodBookDB.getById(
  ...
  static async getJur1Data(data) {
    for (let main_schet of data.main_schets) {
      ...
      const prixod_data = await PrixodBookDB.getJur1Prixod(
        [data.region_id, main_schet.id, data.prixod_schets],
  ...
  static async getJur2Data(data) {
    for (let main_schet of data.main_schets) {
      ...
      const prixod_data = await PrixodBookDB.getJur2Prixod(
        [data.region_id, main_schet.id, data.prixod_schets],
        { from: data.from, to: data.to },
        data.operator
      );
  ...
  static async getJur3Data(data) {
    for (let main_schet of data.main_schets) {
      ...
      const prixod_data = await PrixodBookDB.getJur3Prixod(
        [data.region_id, main_schet.id, data.prixod_schets],
        { from: data.from, to: data.to },
        data.operator
      );
  ...
  static async getJur7Data(data) {
    const prixod_data = await PrixodBookDB.getJur7Prixod(
      [data.region_id, data.budjet_id, data.prixod_schets],
      { from: data.from, to: data.to },
      data.operator
    );
  ...
  static async getFromData(data) {
    ...
    for (let schet of data.schets) {
      ...
      const summa = schet.prixod - schet.prixod;

      if (summa > 0) {
        schet.prixod = summa;
        schet.prixod = 0;
      } else if (summa < 0) {
        schet.prixod = Math.abs(summa);
        schet.prixod = 0;
      } else {
        schet.prixod = 0;
        schet.prixod = 0;
