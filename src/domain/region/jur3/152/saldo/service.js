...
exports.Saldo152Service = class {
  ...
  static async create(data) {
    const result = await db.transaction(async (client) => {
      await Saldo152DB.deleteByMonth(
  ...
  static async get(data) {
    const result = await Saldo152DB.get(
  ...
  static async getById(data) {
    const result = await Saldo152DB.getById(
  ...
  static async getByMonth(data) {
    const result = await Saldo152DB.getByMonth([
      data.main_schet_id,
      data.year,
      data.month,
      data.region_id,
  ...
  static async getFirstSaldo(data) {
    const result = await Saldo152DB.getFirstSaldo([
      data.region_id,
      data.main_schet_id,
      data.schet_id,
    ]);
  ...
  static async getEndSaldo(data) {
    const result = await Saldo152DB.getEndSaldo([
      data.region_id,
      data.main_schet_id,
      data.schet_id,
    ]);
  ...
  static async createSaldoDate(data) {
    ...
    const check = await Saldo152DB.getSaldoDate([
      data.region_id,
      saldo_date,
      data.main_schet_id,
      data.schet_id,
  ...
  static async getDateSaldo(data) {
    const result = await Saldo152DB.getDateSaldo([
      data.region_id,
      data.main_schet_id,
      data.schet_id,
    ]);
