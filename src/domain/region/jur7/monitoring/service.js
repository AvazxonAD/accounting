...
exports.Jur7MonitoringService = class {
  ...
  static async capExcel(data) {
    ...
    // iznos schet
    let iznos_column = column + 2;
    worksheet.addRow({});
    worksheet.addRow({});

    worksheet.addRow({
      prixod: "Износ счет",
      rasxod: "Износ суб счет",
      summa: "Сумма",
    });
    for (let iznos of data.grouped_data) {
      worksheet.addRow({
        prixod: iznos.iznos_schet,
        rasxod: iznos.iznos_sub_schet,
        summa: Math.round(iznos.summa * 100) / 100,
      });
      iznos_column++;
    }
    worksheet.mergeCells(`A${iznos_column}`, `B${iznos_column}`);
    worksheet.getCell(`A${iznos_column}`).value = "Всего";
    worksheet.getCell(`C${iznos_column}`).value = data.to_month_iznos;
    ...
    let itogo_column = Math.max(iznos_column, rasxod_column, column) + 2;

    worksheet.mergeCells(`A${itogo_column}`, `G${itogo_column}`);
    // hard code
    worksheet.getCell(`A${itogo_column}`).value =
      `Остаток к концу дня:               ${HelperFunctions.returnStringSumma(Math.round((data.summa_to - data.to_month_iznos) * 100) / 100)}`;
    itogo_column++;
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      row.eachCell((cell, column) => {
        ...
        if (cellData.bold) {
          ...
          cell.value === "Износ счет" ||
          cell.value === "Износ суб счет"
        ) {
          bold = true;
