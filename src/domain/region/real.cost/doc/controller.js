...
exports.Controller = class {
  static async createDoc(req, res) {
    ...
    const { month, year, type_document, childs } = req.body;
    ...
    for (let child of childs) {
      const smeta_grafik = await SmetaGrafikService.getByIdSmetaGrafik({
        region_id,
      ...
      if (type_document === "start" || type_document === "end") {
        if (child.debet_sum > 0 && child.kredit_sum > 0) {
          return res.error(
            `Only either debit or credit amount can be provided, not both`,
            400
          );
        }
      }
  ...
  static async getDoc(req, res) {
    ...
    const docs = await DocRealCostService.getDocs({
      region_id,
      budjet_id,
      year,
      month,
  ...
  static async updateDoc(req, res) {
    ...
    if (
      old_doc.year !== body.year ||
      old_doc.month !== body.month ||
      old_doc.type_document !== body.type_document
    ) {
      const doc = await DocRealCostService.getByIdDoc({
        region_id,
        budjet_id,
