const { podotchetLitsoValidation } = require("@helper/validation");
...
const getPodotchetLitso = async (req, res) => {
  try {
    ...
    const result = await getAllPodotChetService(
      region_id,
      offset,
      limit,
      search
...
const updatePodotchetLitso = async (req, res) => {
  try {
    ...
    const data = validationResponse(podotchetLitsoValidation, req.body);
    if (
      podotchet_litso.name !== data.name ||
      podotchet_litso.rayon !== data.rayon
    ) {
      await getByAllPodotChetService(data.name, data.rayon, region_id);
    }
...
const getByIdPodotchetLitso = async (req, res) => {
  try {
    const result = await getByIdPodotchetService(
      req.user.region_id,
      req.params.id,
      true
    );
