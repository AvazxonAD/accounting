...
exports.OrganizationService = class {
    ...
    const result = await db.transaction(async (client) => {
      ...
      for (let doc of data.organization_ids) {
        organization_ids.push(
          await OrganizationDB.setParentId([data.parent_id, doc.id], client)
        );
      }
  ...
  static async export(data) {
    ...
    const fileName = `organization.${new Date().getTime()}.xlsx`;
  ...
  static async getById(data) {
    const result = await OrganizationDB.getById(
  ...
  static async import(data) {
    await db.transaction(async (client) => {
      for (let item of data.data) {
        const check = await this.getById({
