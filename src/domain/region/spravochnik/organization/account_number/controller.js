...
exports.Controller = class {
  ...
  static async create(req, res) {
    ...
    const checkAccountNumber = await AccountNumberService.getByAccountNumber({
      raschet_schet,
      spravochnik_organization_id,
    });
    if (checkAccountNumber) {
      return res.error(
        `${req.i18n.t("gaznaExists")} Ganza: ${raschet_schet}`,
        409
      );
    }
  ...
  static async get(req, res) {
    ...
    const { data, total } = await AccountNumberService.get({
      region_id,
      offset,
      ...req.query,
    });
  ...
  static async getById(req, res) {
    ...
    const result = await AccountNumberService.getById({
      region_id,
      id,
      isdeleted: true,
    });
