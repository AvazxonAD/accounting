...
const getByIdMainSchetService = async (
  region_id,
  id,
  ignoreDeleted = false
) => {
...
const checkMainSchetService = async (id) => {
  try {
    const tables = await pool.query(
      `SELECT conrelid::regclass AS name FROM  pg_constraint WHERE confrelid = 'main_schet'::regclass`
    );
    for (let table of tables.rows) {
      ...
      if (test.rows.length > 0) {
        throw new ErrorResponse(
          "Cannot delete this data as it is linked to existing documents",
          400
        );
      }
    }
...
const getByAccountNumberMainSchetService = async (
  region_id,
  account_number
) => {
  try {
    const result = await pool.query(
      ...
      WHERE main_schet.account_number = $1 AND main_schet.isdeleted = false AND regions.id = $2`,
      [account_number, region_id]
    );
    if (result.rows[0]) {
