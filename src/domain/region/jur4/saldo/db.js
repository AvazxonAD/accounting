...
exports.Jur4SaldoDB = class {
  ...
  static async deleteByMonth(params, client) {
    await client.query(
      `--sql
      UPDATE
        podotchet_saldo_child
      SET isdeleted = true
      WHERE id = ANY(
        SELECT
          ch.id
        FROM podotchet_saldo_child ch
        JOIN podotchet_saldo d ON d.id = ch.parent_id
        WHERE d.year = $1
          AND d.month = $2
    ...
    await client.query(
      `UPDATE podotchet_saldo SET isdeleted = true WHERE year = $1 AND month = $2 AND main_schet_id = $3 AND schet_id = $4`,
      params
    );
  }
  ...
  static async cleanData(params, client) {
    const queryParent = `UPDATE podotchet_saldo SET isdeleted = true WHERE main_schet_id = $1 AND schet_id = $2`;
  ...
  static async delete(params, client) {
    const queryParent = `UPDATE podotchet_saldo SET isdeleted = true WHERE id = $1 RETURNING id`;
