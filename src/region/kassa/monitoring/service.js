...
exports.KassaMonitoringService = class {
  ...
  static async cap(data) {
    const schets = await KassaMonitoringDB.getSchets([
      data.region_id,
      data.main_schet_id,
      data.from,
      data.to,
    ...
    for (let schet of schets) {
      schet.summa = await KassaMonitoringDB.getSummaSchet([
        data.region_id,
        data.main_schet_id,
        data.from,
        data.to,
    ...
    const balance_from = await KassaMonitoringDB.getSumma(
      [data.region_id, data.main_schet_id, data.from],
      "<"
    );

    const balance_to = await KassaMonitoringDB.getSumma(
      [data.region_id, data.main_schet_id, data.to],
      "<="
    );
  ...
  static async capExcel(data) {
    const title = `Дневной отчет по ${data.report_title.name} №1. Счет: ${data.main_schet.jur1_schet}. Ҳисоб рақами: ${returnStringSumma(data.main_schet.account_number)}`;
  ...
  static async daily(data) {
    ...
    const balance_from = await KassaMonitoringDB.dailySumma(
      [data.region_id, data.main_schet_id, data.from],
      "<"
    );

    const balance_to = await KassaMonitoringDB.dailySumma(
      [data.region_id, data.main_schet_id, data.to],
      "<="
    );
