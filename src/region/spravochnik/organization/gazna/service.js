...
const {
  BankService,
} = require("../../../../admin/spravochnik/report.title/service");
...
exports.GaznaService = class {
  ...
  static async getByGazna(data) {
    const result = await GaznaDB.getByGazna([
  ...
  static async getById(data) {
    const result = await GaznaDB.getById(
      [data.id],
      data.organ_id,
      data.isdeleted
    );
    ...
    await db.transaction(async (client) => {
      for (let item of data.data) {
        if (item.bank_klient && item.mfo) {
          const bank = await BankService.getByMfoName({
