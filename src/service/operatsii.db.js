...
const createOperatsii = asyncFunctionHandler(async (name,  schet, sub_schet, type_schet) => {
...
const getByNameAndSchetOperatsii = asyncFunctionHandler(async (name, type_schet) => {
    const result = await pool.query(`SELECT * FROM spravochnik_operatsii WHERE name = $1 AND type_schet = $2 AND isdeleted = false
    `, [name, type_schet]);
    return result.rows[0]
})
...
const totalOperatsii = asyncFunctionHandler(async (query) => {
    const result = await pool.query(`SELECT COUNT(id) AS total FROM spravochnik_operatsii WHERE isdeleted = false AND type_schet = $1`, [query]);
    return result.rows[0]
})

const getByIdOperatsii = asyncFunctionHandler(async (id) => {
    let result = await pool.query(`SELECT * FROM spravochnik_operatsii WHERE id = $1 AND isdeleted = false
    `, [id]);
    return result.rows[0]
})


const updateOperatsii = asyncFunctionHandler(async (name, schet, sub_schet, type_schet, id) => {
...
const deleteOperatsii = asyncFunctionHandler(async (id) => {
    const result = await pool.query(`UPDATE spravochnik_operatsii SET isdeleted = $1 WHERE id = $2 RETURNING *`, [true, id])
    return result.rows[0]
})
