...
const getByIdAuth = asyncFunctionHandler(async ( id ) => {
    const result = await pool.query(`SELECT * FROM users WHERE id = $1 AND isdeleted = false`, [ id ])
    return result.rows[0]
})

const updateAuth = asyncFunctionHandler(async (login, password, fio, id) => {
    const result = await pool.query(`UPDATE users SET login = $1, password = $2, fio = $3 WHERE id = $4 RETURNING *
...
const getProfileAuth = asyncFunctionHandler(async ( id ) => {
    const  result = await pool.query(`SELECT users.id, role.id AS role_id, role.name AS role_name, users.fio, users.login
        FROM users 
