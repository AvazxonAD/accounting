...
const getByNameRole = asyncFunctionHandler(async ( name ) => {
    const result = await pool.query(`SELECT * FROM role WHERE name = $1 AND isdeleted = false`, [name]);
...
const get_all_role = asyncFunctionHandler(async () => {
    const result = await pool.query(`SELECT id, name FROM role WHERE isdeleted = false ORDER BY id`)
    return result.rows
})

const getByIdRole = asyncFunctionHandler(async ( id ) => {
    const result = await pool.query(`SELECT id, name FROM role WHERE isdeleted = false AND id = $1`, [id])
    return result.rows[0]
})
...
const delete_role = asyncFunctionHandler(async ( id ) => {
    const result = await pool.query(`UPDATE role SET isdeleted = $1 WHERE id = $2 RETURNING *`, [true, id])
    return result.rows[0]
})
