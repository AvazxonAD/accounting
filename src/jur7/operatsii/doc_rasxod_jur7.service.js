...
const updateDocumentJur7DB = async (data) => {
  ...
  try {
    ...
    const document = result.rows[0];
    await client.query(`DELETE FROM document_rasxod_jur7_child WHERE document_rasxod_jur7_id = $1`, [document.id]);
    const queryArray = [];
    for (let child of data.childs) {
      const query = client.query(
        `
...
const deleteDocumentJur7DB = async (id) => {
  try {
    await pool.query(`UPDATE document_rasxod_jur7_child SET isdeleted = $1 WHERE document_rasxod_jur7_id = $2`, [true, id]);
    await pool.query(`UPDATE document_rasxod_jur7 SET isdeleted = true WHERE id = $1 AND isdeleted = false`, [id]);
  } catch (error) {
    throw new ErrorResponse(error, error.statusCode);
  }
}
