...
exports.Jur7MonitoringService = class {
  static async cap(data) {
    ...
    result.schets = await Jur7MonitoringDB.getSchetsForCap([
      data.budjet_id,
      data.region_id,
      data.from,
      data.to,
    ...
    for (let schet of result.schets) {
      schet.summa = await Jur7MonitoringDB.getCapSchetSumma([
        data.budjet_id,
        data.region_id,
        data.from,
        data.to,
    ...
    result.debet_sub_schets = await Jur7MonitoringDB.getDebetSubSchetsForCap([
      data.budjet_id,
      data.region_id,
      data.from,
      data.to,
    ...
    for (let schet of result.debet_sub_schets) {
      schet.summa = await Jur7MonitoringDB.getDebetSubSchetSumma([
        data.budjet_id,
        data.region_id,
        data.from,
        data.to,
        schet.debet_sub_schet,
      ]);
      result.debet_sub_schets_summa += schet.summa;
    }

    result.kredit_sub_schets = await Jur7MonitoringDB.getKreditSubSchetsForCap([
      data.budjet_id,
      data.region_id,
      data.from,
      data.to,
    ...
    for (let schet of result.kredit_sub_schets) {
      schet.summa = await Jur7MonitoringDB.getKreditSubSchetSumma([
        data.budjet_id,
        data.region_id,
        data.from,
        data.to,
        schet.kredit_sub_schet,
      ]);
      result.kredit_sub_schets_summa += schet.summa;
    }

    result.iznos_schets =
      await Jur7MonitoringDB.getIznosSummaByProvodkaSubSchet([
        data.budjet_id,
        data.region_id,
        data.from,
        data.to,
  ...
  static async capExcel(data) {
    ...
    worksheet.getCell("A2").value =
      `${HelperFunctions.returnStringDate(new Date(data.from))} дан   ${HelperFunctions.returnStringDate(new Date(data.to))} гача`;
  ...
  static async backCap(data) {
    const result = await Jur7MonitoringDB.getSchetsForBackCap([
      data.budjet_id,
      data.region_id,
      data.from,
      data.to,
    ...
    for (let item of result.kredit_schets) {
      item.summa = await Jur7MonitoringDB.getBackCapSchetSumma(
        [data.budjet_id, data.region_id, data.from, data.to, item.schet],
        "kredit_schet"
      );
  ...
  static async getSaldo(data) {
    let { products, total } = await Jur7MonitoringDB.getProducts(
      [data.responsible_id, data.offset, data.limit],
      data.product_id,
      data.search
    );

    for (let product of products) {
      product.from = await Jur7MonitoringDB.getKolSumma(
        [product.naimenovanie_tovarov_jur7_id, data.responsible_id],
        data.from
      );
      product.internal = await Jur7MonitoringDB.getKolSumma(
        [product.naimenovanie_tovarov_jur7_id, data.responsible_id],
        null,
        [data.from, data.to]
      );
      product.to = await Jur7MonitoringDB.getKolSumma(
        [product.naimenovanie_tovarov_jur7_id, data.responsible_id],
        data.to
      );
      ...
      product.prixod_data = await Jur7MonitoringDB.getPrixodInfo([
        product.naimenovanie_tovarov_jur7_id,
      ]);
