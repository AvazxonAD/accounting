...
const { getAllMonitoring, prixodRasxodPodotchetService, podotchetMonitoringService } = require('./podotchet.monitoring.service')
...
const podotchetMonitoring = async (req, res) => {
    try {
        const { limit, page, main_schet_id, from, to, operatsii_id } = validationResponse(podotchetQueryValidation, req.query)
        const region_id = req.user.region_id;
        const offset = (page - 1) * limit;
        await getByIdMainSchetService(region_id, main_schet_id);
