...
var createLogger = function createLogger(filename) {
  return winston.createLogger({
    ...
    format: winston.format.combine(winston.format.timestamp({
      ...
      var logMessage = "".concat(timestamp, ". ").concat(meta.type || 'unknown', "."); // Type ni tekshirib qo'shamiz
      if (meta.id && meta.user_id) {
        logMessage += " id:".concat(meta.id, ". user_id:").concat(meta.user_id);
      }
      return logMessage;
    })),
...
var logRequest = function logRequest(type_log, meta) {
  if (type_log === 'get') {
