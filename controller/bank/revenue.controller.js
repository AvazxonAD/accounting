...
exports.create_revenue = asyncHandler(async (req, res, next) => {
    ...
    for(let revenue of revenues){
        ...
        let goal = await pool.query(`SELECT * FROM goals WHERE id = $1 AND user_id = $2`, [revenue.goal_id, user_id]);
        goal = goal.rows[0];
        if (!goal) {
    ...
    for(let revenue of revenues){
        ...
        if (result.rows[0]) {
            await pool.query(`UPDATE requisites SET balance = $1 WHERE user_id = $2 AND id = $3`, [Number(requisite.balance) + Number(revenue.contract_summa), user_id, requisite.id]);
        }
    }
