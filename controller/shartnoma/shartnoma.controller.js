...
const { createShartnoma, getAllShartnoma, getTotalShartnoma, updateShartnoma, getByIdShartnoma } = require("../../service/shartnoma.db");
...
const create = asyncHandler(async (req, res, next) => {
    ...
    const test_organization = await getByIdOrganization(user_id, spravochnik_organization_id)
    if (!test_organization) {
        return next(new ErrorResponse('Hamkor topilmadi', 500));
    }

    
    const shartnoma = await createShartnoma(user_id, doc_num, doc_date, summa, opisanie, smeta_id, smeta_2, spravochnik_organization_id)
    if (!shartnoma) {
...
const update_shartnoma = asyncHandler(async (req, res, next) => {
    ...
    const result = await updateShartnoma(id, doc_num, doc_date, summa, opisanie, smeta_id, smeta_2, spravochnik_organization_id)
    if (!result) {
