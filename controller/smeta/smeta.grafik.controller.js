...
const create = asyncHandler(async (req, res, next) => {
    ...
    const smeta = await pool.query(`SELECT * FROM smeta WHERE id = $1 AND isdeleted = false`, [smeta_id])
    if(!smeta.rows[0]){
        return next(new ErrorResponse("Server xatolik smeta topilmadi", 500))
    }

    const budjet = await pool.query(`SELECT * FROM spravochnik_budjet_name WHERE id = $1 AND isdeleted = false`, [spravochnik_budjet_name_id])
    if(!budjet.rows[0]){
        return next(new ErrorResponse("Server xatolik budjet topilmadi", 500))
    }

    const test = await pool.query(`SELECT * FROM smeta_grafik 
...
const update = asyncHandler(async (req, res, next) => {
    let value = await pool.query(`SELECT * FROM smeta_grafik 
    ...
    if(Number(itogo) !== sum(oy_1, oy_2, oy_3, oy_4, oy_5, oy_6, oy_7, oy_8, oy_9, oy_10, oy_11, oy_12)){
        return next(new ErrorResponse('Server xatolik. Itogo va oylar summa yig`indisi teng bolishi kerak', 400))
    }
    
    const result = await pool.query(`UPDATE  smeta_grafik 
        SET 
            smeta_id = $1, 
...
const deleteValue = asyncHandler(async (req, res, next) => {
    let value = await pool.query(`SELECT * FROM smeta_grafik WHERE id = $1 AND isdeleted = false AND user_id = $2
    ...
    const deleteValue = await pool.query(`UPDATE smeta_grafik SET isdeleted = $1 WHERE id = $2 RETURNING *`, [true, req.params.id])
