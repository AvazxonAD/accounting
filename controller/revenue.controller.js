...
exports.create_revenue = asyncHandler(async (req, res, next) => {
    ...
    for (let revenue of revenues) {
        ...
        for (let goal of revenue.goals) {
            if (!goal.goal_id) {
                return next(new ErrorResponse('Maqsad bo`sh qolishi mumkin emas', 400));
            }
        }

        if (
            typeof revenue.partner_id !== "number" ||
            typeof revenue.contract_date !== "string" ||
            typeof revenue.contract_summa !== "number" ||
            !Array.isArray(revenue.goals) ||
            typeof revenue.contract_number !== "string" ||
            typeof revenue.goal_info !== "string"
        ) {
        ...
        for (let goal of revenue.goals) {
            let test = await pool.query(`SELECT * FROM goals WHERE id = $1 AND user_id = $2`, [goal.goal_id, req.user.id]);
            if (!test.rows[0]) {
    ...
    for (let revenue of revenues) {
        ...
        const result_goals = [];
        for (let goal of revenue.goals) {
            const goal_obj = await pool.query(`SELECT id, name, short_name, schot, number FROM goals WHERE user_id = $1 AND id = $2`, [req.user.id, goal.goal_id]);
        ...
        const result = await pool.query(
            ...
            requisite_id, inn, name, mfo, bank_name, account_number, treasury_account_number, 
            shot_number, budget, partner_id, partner_name, partner_bank_name, partner_account_number, 
            partner_mfo, partner_inn, contract_number, 
            contract_date, contract_summa, goals, goal_info, user_id
        ) 
        ...
        if (result.rows[0]) {
            total_summa += Number(revenue.contract_summa);
        }
    }

    await pool.query(
        `UPDATE requisites SET balance = $1 WHERE user_id = $2 AND id = $3`,
        [Number(requisite.balance) + total_summa, req.user.id, requisite.id]
    );
