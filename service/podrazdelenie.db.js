...
const getByAllPodrazdelenie = asyncFunctionHandler(async (user_id, name, rayon) => {
...
const createPodrazdelenie = asyncFunctionHandler(async (user_id, name, rayon) => {
    const result = await pool.query(`INSERT INTO spravochnik_podrazdelenie(name, rayon, user_id) VALUES($1, $2, $3) RETURNING *
    `, [name, rayon, user_id]);
    return result.rows[0]
})

const getAllPodrazdelenie = asyncFunctionHandler(async (user_id, offset, limit) => {
...
const getTotalPodrazlanie = asyncFunctionHandler(async (user_id) => {
    const result = await pool.query(`SELECT COUNT(id) AS total FROM spravochnik_podrazdelenie WHERE isdeleted = false AND user_id = $1`, [user_id]);
    return result.rows[0]
})

const getByIdPodrazlanie = asyncFunctionHandler(async (user_id, id) => {
    const result = await pool.query(`SELECT * FROM spravochnik_podrazdelenie   WHERE id = $1 AND user_id = $2 AND isdeleted = false`, [id, user_id])
    return result.rows[0]
})

const updatePodrazlanie = asyncFunctionHandler(async (user_id, id, name, rayon) => {
...
const deletePodrazlanie = asyncFunctionHandler(async (id) => {
    const result = await pool.query(`UPDATE spravochnik_podrazdelenie SET isdeleted = $1 WHERE id = $2 RETURNING *`, [true, id])
    return result.rows[0]
})
