...
const getTotalSostav = asyncFunctionHandler(async (user_id) => {
    const result = await pool.query(`SELECT COUNT(id) AS total FROM spravochnik_sostav WHERE isdeleted = false AND user_id = $1`, [user_id]);
    return result.rows[0]
})

const getByIdSostav = asyncFunctionHandler(async (user_id, id) => {
    const result = await pool.query(`SELECT * FROM spravochnik_sostav   WHERE id = $1 AND user_id = $2 AND isdeleted = false`, [id, user_id])
    return result.rows[0]
})
...
const deleteSostav = asyncFunctionHandler(async (id) => {
    const result = await pool.query(`UPDATE spravochnik_sostav SET isdeleted = $1 WHERE id = $2 RETURNING *`, [true, id])
    return result.rows[0]
})
